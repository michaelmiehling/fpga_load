...
static char RCSid[]="$Header: /dd2/CVSR/COM/TOOLS/FPGA_LOAD/COM/fpga_load.c,v 1.20 2010/08/26 09:57:00 CKauntz Exp -C3 test ts 09.03.2010- $";

#include "fpga_load.h"
#include "fpga_load_flash.h"
#include "MEN/smb2.h"
#warning TEST WITH CORRECT LEN
/*-----------------------------------------+
 |  GLOBALS                                |
 +-----------------------------------------*/
...
static void Usage(void)
{
	printf( "\n\nUsage: fpga_load [options]\n"
			"Function: manage FPGA configurations over PCI bus\n"
			"Device settings:\n"
			"   [vendorID          PCI vendor ID Nr.( e.g. 0x1172)\n"
			"    deviceID          PCI device ID Nr.( e.g. 0x4D45 for chameleon_2 device)\n"
			"    subSystemVendorID PCI subsystem vendor ID Nr. (e.g. 0x0007 for 15P018)\n"
			"    instance          instance of device found\n"
			"                      to see all instances use option -s\n"
			"    [-b <nr>          BAR: 0..5                              [0]\n"
			"     -o <offset>]]    FPGA module register offset from BAR   [0]\n"
			"   [-a <VME addr>]    directly specify VME address\n"
			"   [-d <addr>]        directly specify address\n"
			"                      where registers can be found\n"
			" When [-b xxx -o xxx] or [-a] or [-d] is specified, the register\n"
			" addresses are not read from the Chameleon table but set manually from this options\n"
			"    -z                use only SPI Flashes with 16Z126 SPI Flash Interface\n"
			"    -p <SMB host>     use PLD Flash interface (SMB) instead of 16Z045_FLASH\n"
			"                      the SMB host can be one of:\n"
			"                      menz001 (more to come)\n"
			"    -u <file> <nr>    update FPGA config Nr. <0-3> in flash\n"
			"                      offset in flash will be read from fpga header in flash\n"
			"                      also header information will be validated before update\n"
			"                      If this option is selected, all other options are ignored\n"
			"                      \n"
			"    -r                read block of data from flash\n"
			"        <startAddr>   offset address from the beginning of the\n"
			"                      FPGA config memory space              [0x00]\n"
			"        <length>      number of bytes to read\n"
			"       [<file>]       if file is specified, data is written to file\n"
			"                      otherwise data is printed to stdout\n"
			"    -f                force mode !!!!! only use if you know what you are doing !!!!!!\n"
			"                      no validation of any header information will be performed\n"
			"                      all options must be passed by command line options\n"
			"                      full access to flash commands is granted\n"
			"                      more than one command can be passed,\n"
			"    	               they will be executed sequentially from first to last\n"
			"        -c                erase chip\n"
			"        -e                erase sectors\n"
			"            <startAddr>   any address in the first block to erase\n"
			"            <len>         length to erase (bytes)                [0]\n"
...
#endif
			"    -v                verbose mode                          [no]\n"
			"    -s                show all pci devices only             [no]\n"
			...
			"\n"
			"(c) 2004 by MEN Mikro Elektronik GmbH, Nuremberg, Germany\n%s\n\n",RCSid);
}
...
/*	int be_flag = 0; */
...
#ifdef VXWORKS
	if ((errstr = UTL_ILLIOPT("obadsurltfwv?himecpxz", errbuf)))
#else
	if ((errstr = UTL_ILLIOPT("obadsurltfwv?hecpxz", errbuf)))
#endif
	{
		printf( "*** ERROR: %s\n" FPGA_P_HELP, errstr);
		error = 1;
    ...
    /* Use the Serial SPI Flash interface 16Z126 type for the SPI flashes */
    if ( UTL_TSTOPT("z") ){
	    h->interfacespi = 1;
	}else
	{
		...
		while( largc )
		{
			if( !strcmp( "-r", *largv ) )
			{
				if((error = Read_block(largc, largv, h, osHdl, &rc)))
					printf( "ERROR: ReadBlock: error code 0x%04X\n",
							(unsigned int)(error));
				goto MAINEND;
			}
			largc--;
		...
		while( largc ){
			if( !strcmp( "-x", *largv ) ) {
				if((error = Set_BusSwitch(largc, largv, h, osHdl, &rc)))
					printf( "ERROR: Set_BusSwitch: error code 0x%04X\n",
							(unsigned int)(error));
				goto MAINEND;
			}
			largc--;
	...
	while( argc > 0 ){
		...
		if( !strcmp( "-c", *argv ) ) {
			...
			printf( "ERROR: EraseChip: error code 0x%04X\n",
					(unsigned int)(error));
			goto MAINEND;
		}
		/* erase block */
		if( !strcmp( "-e", *argv ) ) {
			...
			printf( "ERROR: EraseSectors: error code 0x%04X\n",
					(unsigned int)(error));
			goto MAINEND;
		}
		/* read block */
		...
		if( !strcmp( "-w", *argv ) ) {
			...
			printf( "ERROR: WriteBlock: error code 0x%04X\n",
					(unsigned int)(error));
			goto MAINEND;
		}
		...
		/* nothing matched !!!!!!! */
		printf("parameter not understood: %s\n", *argv);
...
READEND:
	if( error ) {
		printf("Read_block failed: 0x%04x\n", (unsigned int)error);
		h->flash_entry.Reset(&h->flashDev);
	}
...
/*	int32 be_flag = 0; */
	...
	*retCnt = 1;
	...
	if (argc<2 || !sscanf(argv[1],"%s", fileName)) {
		printf( "\nOption -w requires at least the file to be loaded\n");
		error = ERR_UOS_ILL_PARAM;
		goto WRITEEND;
									 ...
									 &fBufGotSize )) < 0 )
	{
		printf("Please specify correct path/filename\n");
		error = ERR_UOS_ILL_PARAM;
		goto WRITEEND;
	}
	(*retCnt)++;
...
static void Trigger_fpga_load(DEV_HDL *h)
{
	...
	if( h->mapType )
		...
		printf( "Trigger_fpga_load: IO mapped access not supported\n");
...
/*	int32 be_flag = 0; */
	...
	*retCnt = 2;
	...
	if (argc<2 ||
		...
		printf("\noption -u requires filename (see \"fpga_load -h\")\n");
...
/*	int32 be_flag = 0; */
	...
	*retCnt = 2;
	...
	if ( argc < 2 ||									/* no bus switch?     */
		...
		printf("\noption -x requires bus switch [1/2/3] (see \"fpga_load -h\")\n");
		return (ERR_UOS_ILL_PARAM);
	}
...
SETBUSSWEND:
	if( error ) {
		printf("Set_BusSwitch failed: 0x%04x\n", (unsigned int)error);
		h->flash_entry.Reset(&h->flashDev);
	}
...
)
{
	...
	if( !flashInterf ) /* 16Z045_FLASH  or 16Z126_SPI_FLASH */
	{
		if( (error = OSS_MapPhysToVirtAddr(osHdl,
							 ...
							 (void**)&h->mappedAddr)) )
		{
			printf( "ERROR: Init_Flash: can't map address (0x%08x)\n",
					(int)error);
	...
	{
		if( !h->smbLocHdl.smbHdl ) {
			printf( "Init_Flash: SMBus Handle not initialized\n");
			return( ERR_NO_SUPPORTED_FLASH_DEVICE_FOUND );
		}
		h->flash_acc_size = Z100_FLASH_ACCESS_16BIT;
	} else {
		printf("ERROR: Init_Flash: no supported flash interface\n");
	}
					 ...
					 u_int8 verbose )
{
	...
	CHAM_FUNCTBL		chaMemFktTbl, *chaFktTbl = NULL;
	CHAM_FUNCTBL		chaMemSwFktTbl;
#ifdef Z100_IO_ACCESS_ENABLE
	...
	{
		printf( "No chameleon table found for PCI device %d/%d/%d "
				"(bus/dev/fun) (error=0x%08lx)\n",
	...
	if( chaFktTbl->Info( chaHdl, chamInfo ) != 0 )	{
		printf( "Cham_Info failed for device %d/%d/%d (bus/dev/fun)\n",
				pciDev->bus, pciDev->dev, pciDev->fun);
		goto ERROR_END;
	}
	if (verbose) {
		printf( "Cham_Info for device %d/%d/%d (bus/dev/fun):\n"
				" chaRev: %d;	busId: %d;	tableNbr %d;\n"
	...
	for( i=0; i<1000; i++ ){
		...
		if(ret != 0) {
			printf( "Cham_TableIdent failed for device %d/%d/%d (bus/dev/fun)\n",
					pciDev->bus, pciDev->dev, pciDev->fun);
			goto ERROR_END;
		}
		...
		if(verbose)
			printf( "CHAMELEONV2_HEADER #%2d:\n"
					"  busType=0x%02x, busId=%d, model=%c, revision=0x%02x,\n"
	...
	for( i=0; i<1000; i++ ){
		...
		if(ret != 0) {
			printf( "Cham_UnitIdent failed for device %d/%d/%d (bus/dev/fun)\n",
					pciDev->bus, pciDev->dev, pciDev->fun);
			goto ERROR_END;
		}
...
)
{
	...
	if( (error = OSS_MapVmeAddr( osHdl,
								...
								addrWinHdl )) )
	{
		printf("*** ERROR OSS_MapVmeAddr failed (0x%08x)\n", error);
		goto VMEINITEND;
	}
	...
	if( (error = OSS_BusToPhysAddr( osHdl,
									...
									MAP_REG_SIZE )) )
	{
		printf( "*** ERROR OSS_BusToPhysAddr failed (0x%08x)\n", error);
		goto VMEINITEND;
	}
...
)
{
	...
	if( !strcmp(h->smbLocHdl.smbCtlName, "menz001") )
	{
		if( (error = OSS_MapPhysToVirtAddr(osHdl,
						 ...
						 (void**)&h->mappedAddr)) )
		{
			printf( "ERROR: Z100_SmbInit: can't map address (0x%08x)\n",
					(int)error);
			goto SMBINITEND;
		}
		h->mappedSize = MAP_REG_SIZE;
		...
		{
			printf( "ERROR: Z100_SmbInit: init Z001 ctrl. failed (0x%08x)\n",
					(int)error);
		}
...
)
{
	...
	{
		printf( "ERROR: Z100_SmbExit: exit SMB ctrl. failed (0x%08x)\n",
				(int)error);
		goto SMBEXITEND;
	}
